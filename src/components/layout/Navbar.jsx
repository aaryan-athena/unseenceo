...
import { Menu, Search, LogOut, ChevronDown, User as UserIcon, Globe } from 'lucide-react';
...
export default function Navbar() {
  ...
  const roleLabel = userType === 'venture' ? t('entrepreneur') : userType === 'funder' ? t('funder') : '';
      ...
      <div className="flex items-center gap-2">
        ...
        <div className="relative" ref={langRef}>
          <button
            ...
            className="flex items-center gap-1.5 hover:bg-warm-50 rounded-lg px-2 py-1.5 transition-colors"
            title={t('language')}
          >
            <Globe size={16} className="text-warm-500" />
            <span className="text-xs font-semibold text-warm-700 hidden sm:block">{currentLang?.nativeLabel}</span>
            <ChevronDown size={12} className="text-warm-400 hidden sm:block" />
          </button>

          {langOpen && (
            <div className="absolute right-0 top-full mt-1.5 w-52 bg-white border border-warm-200 rounded-xl shadow-lg py-1 z-50 max-h-80 overflow-y-auto">
